...
def AupdateDiag(W, EmpCov):
    """ Update A matrix assuming latent variables have diagonal covariance
    structure.
    """
...
def AupdateNonDiag(W, EmpCov):
    """ Update A matrix without assuming latent variables have diagonal
    ...
    if np.min(np.linalg.eig(Anew)[0]) <= 0:
        Anew += np.eye(W.shape[1])* (np.abs(np.min(np.linalg.eig(Anew)[0]))
                                     + .001)
    return Anew 


def armijoUpdateW_MultiSubject_penalized(
        W, Wgrad, Gtilde, Shat, alpha=.5, c=0.001, useStiefel=False,
        maxIter=1000):
    """ Update projection matrix, W, using armijo backtracking.

    Gtilde is the matrix: 0.5 A*A - A where A= G/(vI+ G^{-1})
    We just project onto non-negative here, not onto the set of orthogonal,
    non-negatives as this is too restrictive!
    Here both Gtilde and Shat are lists.
    """
    nSub = len(Shat)
    ...
    while stopBackTracking==False:
        ...
        for i in range(nSub):
            ...
            newObj  += (
                np.diag(Wnew.T.dot(Shat[i]).dot(Wnew).dot(Gtilde[i])).sum())
        if (newObj <= currObj + c * alpha * (np.diag(np.diag(
                Wgrad.T.dot(Wnew - W))).sum() + .001)):
...
def nonNegativeCovFactor_LagrangeMult(
        Shat, k=2, diagG=False, lagParam=1, tol=.01, alphaArmijo=0.5,
        maxIter=1000):
    """ Decompose covariance into non-negative W and diagonal G.

    The model proposed by Monti & Hyvarinen (2018) is as follows:

    - we have observations X_1, \ldots, X_n which following a zero mean
      Gaussian with covariance \Sigma
    - we model \Sigma = W G W^T + I for orthonormal W of rank K and dense
      G (k by k)
    - we propose to learn W and G via score matching with some constraints
      on W
    - we introduce non-negativity and orthonormality constraints on W in order
      to ensure the model is identifiable.
    - we note that directly optimizing over the set of non-neg orthonormal
      matricies is too difficult (depends very highly on the initial choice
      of W!). As a result, we employ augmented Lagragian multipliers and
      enforce orthonormality only in the limit (non-negativity is enforced
      at each iteration).

    Parameters
    ----------
    Shat: list of array
        list with each entry a square covariance matrix.
    k: int, default 2
        rank of approximation (dimensionality of latent variables).
    diagG: bool, default False
        should latent variables have diagonal covariance structure.
    lagParam: int, default 1
        coefficient for augmented lagrangian.
    alphaArmijo: float, default 0.5
        backtracking parameter for Armijo rule (stepsize for W updates).
    ...
    -------
    W: array
        non-negative orthonormal loading matrix.
    G: list of array
        each entry a square latent variable covariance.
    iter: int
    ...
    for iter_ in range(maxIter):
        ...
        W = armijoUpdateW_MultiSubject_penalized(
            W=W, Wgrad=Wgrad, Gtilde=AtildeAll, Shat=Shat, alpha=alphaArmijo,
            c=cArmijo, useStiefel=False)
        W = ProjectNonNegative(W) # to ensure non-negativity 
        W = normalizeColumns(W)

        # -------- update A matrices --------
        if diagG:
            A = [AupdateDiag(normalizeColumns(ProjectMax1(W)), Shat[i])
                 for i in range(nSub)]
        else:
            A = [AupdateNonDiag(normalizeColumns(ProjectMax1(W)), Shat[i])
                 for i in range(nSub)]
    ...
    if diagG:
        G = [np.diag(np.diag(W.T.dot(Shat[i]).dot(W))) - np.eye(k)
             for i in range(nSub)]
    else:
        G = [W.T.dot(Shat[i]).dot(W) - np.eye(k) for i in range(nSub)]

    # finally we compute the data likelihood
    PresHat, logLik = [], []
    for i in range(nSub):
        PresHat.append(
            np.eye(p) - W.dot(G[i]).dot(np.linalg.pinv(
                G[i] + np.eye(k))).dot(W.T))
        logLik.append(
            0.5 * np.log(np.linalg.det(PresHat[i])) - 0.5 *
            np.sum(np.diag(Shat[i].dot(PresHat[i]))))

    return {'W': W, 'G': G, 'iter': iter_, 'logLik': logLik}
